...
// ─── Driver Card for real API rides ───────────────────────
  ...
  return (
    ...
    >
      <div style={{ position: "relative", height: "160px", background: "#F1F5F9" }}>
        <img src={driverPhoto} alt={driverName} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(driverName)}&background=14B8A6&color=fff&size=200`; }} />
        <div style={{ position: "absolute", top: "10px", right: "10px", background: "rgba(255,255,255,0.95)", borderRadius: "20px", padding: "4px 10px", fontSize: "12px", fontWeight: "700", color: "#0F2D52", display: "flex", alignItems: "center", gap: "4px" }}>⭐ {rating > 0 ? rating.toFixed(1) : "New"}</div>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ fontSize: "12px", color: "#14B8A6", fontWeight: "600", marginBottom: "10px" }}>{ride.source?.address} → {ride.destination?.address}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "12px" }}>
          {tags.map(t => <span key={t} style={{ background: "#F0FDFA", border: "1px solid #99F6E4", color: "#0F6E56", fontSize: "11px", padding: "2px 8px", borderRadius: "20px" }}>{t}</span>)}
          {tags.length === 0 && <span style={{ background: "#F0FDFA", border: "1px solid #99F6E4", color: "#0F6E56", fontSize: "11px", padding: "2px 8px", borderRadius: "20px" }}>🚘 {ride.availableSeats} seats left</span>}
        </div>
...
// ─── Card for local (offline-published) rides ─────────────
function LocalRideCard({ ride }) {
  const navigate = useNavigate();
  const tags = [];
  const p = ride?.preferences || {};
  if (p.petsAllowed)     tags.push("Pet Friendly");
  ...
  return (
    ...
    >
      {/* Avatar banner */}
      <div style={{ height: "160px", background: "linear-gradient(135deg,#E8FAF9,#B2EDEA)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative" }}>
        <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "#14B8A6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", color: "#fff", fontWeight: "700" }}>
          {(ride.driverName || "D")[0].toUpperCase()}
        </div>
        <div style={{ marginTop: "8px", fontSize: "13px", fontWeight: "700", color: "#0F2D52" }}>{ride.driverName || "Driver"}</div>
        <div style={{ position: "absolute", bottom: "10px", left: "10px", background: "#14B8A6", color: "#fff", borderRadius: "20px", padding: "3px 10px", fontSize: "11px", fontWeight: "600" }}>🚘 {ride.vehicle?.name || "Car"}</div>
      </div>

      <div style={{ padding: "14px 16px" }}>
        <div style={{ fontSize: "12px", color: "#14B8A6", fontWeight: "600", marginBottom: "6px" }}>{ride.from} → {ride.to}</div>
        <div style={{ fontSize: "11px", color: "#64748B", marginBottom: "10px" }}>📅 {ride.date} · ⏰ {ride.time}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "12px" }}>
          {tags.map(t => <span key={t} style={{ background: "#F0FDFA", border: "1px solid #99F6E4", color: "#0F6E56", fontSize: "11px", padding: "2px 8px", borderRadius: "20px" }}>{t}</span>)}
          <span style={{ background: "#F0FDFA", border: "1px solid #99F6E4", color: "#0F6E56", fontSize: "11px", padding: "2px 8px", borderRadius: "20px" }}>🧍 {ride.seats} seats</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
...
function SkeletonCard() {
  return (
    <div style={{ background: "#fff", borderRadius: "20px", overflow: "hidden", border: "1px solid #E5E7EB" }}>
      ...
      <div style={{ padding: "14px 16px" }}>
        {[80, 60, 100].map((w, i) => <div key={i} style={{ height: "12px", background: "#F1F5F9", borderRadius: "6px", width: `${w}%`, marginBottom: "10px" }} />)}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  // Load local rides immediately (no async needed)
  useEffect(() => {
    setLocalRides(getLocalRides());
  }, []);

  // Fetch API rides (may fail if backend has no rides / driver verification issues — that's fine)
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
  ...
  // Merge: local rides first, then API rides, cap at 6
  const allRides = [...localRides, ...apiRides].slice(0, 6);

  const styles = {
    homePage:      { padding: isMobile ? "12px" : "24px", background: "#F8FAFC", minHeight: "100vh", overflowX: "hidden" },
    actionSection: { display: "flex", justifyContent: "center", gap: "18px", marginTop: "-35px", marginBottom: "32px", position: "relative", zIndex: 20, flexWrap: "wrap" },
    actionCard:    { width: isMobile ? "100%" : "420px", height: "82px", background: "#FFFFFF", borderRadius: "16px", padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: "12px", transition: "all 0.25s ease", boxShadow: "0 4px 16px rgba(15,45,82,0.10)", border: "1px solid #E5E7EB", position: "relative" },
    contentSection:{ display: "flex", flexDirection: isTablet || isMobile ? "column" : "row", gap: "24px", marginTop: "16px", alignItems: "flex-start" },
    driversGrid:   { width: "100%", flex: 1, display: "grid", gridTemplateColumns: isMobile ? "1fr" : isTablet ? "repeat(2,1fr)" : "repeat(3,1fr)", gap: "20px" },
    rightPanel:    { width: isTablet || isMobile ? "100%" : "320px", display: "flex", flexDirection: "column", gap: "20px", flexShrink: 0 },
  };
      ...
      <div style={styles.contentSection}>
        <div style={{ flex: 1 }}>
          ...
          <div style={styles.driversGrid}>
            {loading && apiRides.length === 0 && localRides.length === 0 && Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}
            ...
            {/* Local rides first */}
            {localRides.map(ride => <LocalRideCard key={ride.id} ride={ride} />)}

            {/* Then API rides */}
            {!loading && apiRides.map(ride => <RealDriverCard key={ride._id} ride={ride} />)}
          </div>
        </div>
