...
// Global responsive rules. Inline styles can't express media queries, so the
// handful of layout rules that need to change per-breakpoint live here as
// classes, while the bulk of the visual styling stays inline as before.
const ResponsiveStyles = () => (
  <style>{`
    .vd-header-inner { padding: 16px 24px; }
    .vd-container { padding: 32px 24px; }
    .vd-top-row { flex-wrap: nowrap; }
    .vd-help-text { display: inline; }
    .vd-two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
    .vd-amenities-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .vd-row { flex-wrap: nowrap; }
    .vd-row-input { width: 180px; }
    .vd-row-input-num { width: 80px; }
    .vd-actions { flex-direction: row; }
    .vd-step-labels { display: inline; }
    ...
    @media (max-width: 640px) {
      .vd-header-inner { padding: 12px 16px; }
      .vd-container { padding: 20px 16px; }
      .vd-help-text { display: none; }
      .vd-title { font-size: 22px; }
      .vd-amenities-grid { grid-template-columns: 1fr; }
      .vd-row { flex-wrap: wrap; row-gap: 6px; }
      .vd-row-input { width: 100%; text-align: left; }
      .vd-row-input-num { width: 100%; text-align: left; }
      .vd-actions { flex-direction: column; }
      .vd-step-labels { display: none; }
      .vd-step-bar { justify-content: space-between; }
    }
  `}</style>
);
...
function StepBar({ current }) {
  ...
  return (
    <div className="vd-step-bar" style={{ display:"flex", alignItems:"center" }}>
      {steps.map((label,i) => (
        <div key={label} style={{ display:"flex", alignItems:"center", flex: i < steps.length-1 ? 1 : "0 0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
            ...
            <span className="vd-step-labels" style={{ fontSize:12, fontWeight: i===current?700:500, color: i===current?colors.teal:colors.gray, whiteSpace:"nowrap" }}>{label}</span>
          </div>
          {i<steps.length-1 && <div style={{ flex:1, height:2, background: i<current?colors.teal:colors.border, margin:"0 8px", minWidth:12 }}/>}
        </div>
      ))}
    </div>
  );
...
export default function VehicleDetails() {
  ...
  const inputStyle = { fontSize:14, fontWeight:600, color:colors.navy, border:`1.5px solid ${colors.borderTeal}`, borderRadius:6, padding:'4px 8px', backgroundColor:colors.bgTeal, outline:'none', textAlign:'right', boxSizing:'border-box' };
    ...
    <div style={{ minHeight:'100vh', backgroundColor:colors.lightGray, fontFamily:"'Inter', sans-serif" }}>
      ...
      <div style={{ backgroundColor:colors.white, borderBottom:`1px solid ${colors.border}`, position:'sticky', top:0, zIndex:10, boxShadow:'0 1px 8px rgba(0,0,0,0.05)' }}>
        <div className="vd-header-inner" style={{ maxWidth:1280, margin:'0 auto' }}>
          <div className="vd-top-row" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, gap:12 }}>
            <button onClick={() => navigate('/ride-preferences')} style={{ background:'none', border:'none', padding:8, cursor:'pointer', display:'flex', alignItems:'center', flexShrink:0 }}><ChevronLeft style={{ width:24, height:24, color:colors.navy }}/></button>
            <h1 style={{ fontSize:20, fontWeight:700, color:colors.navy, margin:0, textAlign:'center', flex:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>Offer a Ride</h1>
            <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}><HelpCircle style={{ width:20, height:20, color:colors.teal }}/><span className="vd-help-text" style={{ fontSize:14, fontWeight:500, color:colors.navy }}>Need Help?</span></div>
          </div>
          <StepBar current={2} />
        </div>
      </div>

      <div className="vd-container" style={{ maxWidth:1280, margin:'0 auto' }}>
        <h2 className="vd-title" style={{ fontWeight:800, color:colors.navy, marginBottom:4 }}>Vehicle Details</h2>
        ...
        <div className="vd-two-col" style={{ marginBottom:32 }}>
          <div style={{ backgroundColor:colors.white, border:`1px solid ${colors.border}`, borderRadius:12, padding:'20px 24px', minWidth:0 }}>
            ...
            {[
              ...
              <div key={row.field} className="vd-row" style={rowStyle}>
                <span style={{ fontSize:14, color:colors.gray, flexShrink:0 }}>{row.label}</span>
                {row.type === 'select' ? (
                  <select
                    value={vehicleInfo[row.field]}
                    onChange={e => setVehicleInfo(p => ({ ...p, [row.field]: e.target.value }))}
                    className="vd-row-input"
                  ...
                  <input
                    ...
                    value={vehicleInfo[row.field]}
                    onChange={e => setVehicleInfo(p => ({ ...p, [row.field]: e.target.value }))}
                    className={row.type === 'number' ? 'vd-row-input-num' : 'vd-row-input'}
                    style={inputStyle}
                  />
                )}
              </div>
          ...
          <div style={{ backgroundColor:colors.white, border:`1px solid ${colors.border}`, borderRadius:12, padding:'20px 24px', minWidth:0 }}>
            ...
            {['RC Verified','Insurance Valid','PUC Valid','Vehicle Verified by HopIn'].map(item => (
              <div key={item} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 0', borderBottom:`1px solid ${colors.border}`, gap:8, flexWrap:'wrap' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10, minWidth:0 }}>
                  <CheckCircle style={{ width:18, height:18, color:colors.teal, flexShrink:0 }}/><span style={{ fontSize:14, color:colors.navy, fontWeight:500 }}>{item}</span>
                </div>
                <span style={{ fontSize:12, fontWeight:600, color:colors.teal, backgroundColor:colors.bgTeal, border:`1px solid ${colors.borderTeal}`, padding:'3px 10px', borderRadius:20, flexShrink:0 }}>Verified</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ backgroundColor:colors.white, border:`1px solid ${colors.border}`, borderRadius:12, padding:'16px 20px' }}>
            <div className="vd-amenities-grid">
              {amenitiesList.map(({ id, label, icon: Icon }) => (
                <label key={id} onClick={() => toggleAmenity(id)} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', border:`1px solid ${amenities[id]?colors.borderTeal:colors.border}`, borderRadius:10, cursor:'pointer', backgroundColor:amenities[id]?colors.bgTeal:colors.white, transition:'all 0.2s', userSelect:'none', minWidth:0 }}>
                  <Icon style={{ width:20, height:20, color:colors.teal, flexShrink:0 }}/>
                  <span style={{ fontSize:14, color:colors.navy, fontWeight:500, flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{label}</span>
        ...
        <div className="vd-actions" style={{ display:'flex', gap:12 }}>
          <button onClick={() => navigate('/ride-preferences')} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'13px 24px', border:`1.5px solid ${colors.border}`, borderRadius:10, fontSize:15, fontWeight:600, color:colors.navy, backgroundColor:colors.white, cursor:'pointer' }}>
